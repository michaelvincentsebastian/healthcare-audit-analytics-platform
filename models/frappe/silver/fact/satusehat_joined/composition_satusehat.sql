MODEL (
  name silver.composition_satusehat,
  kind FULL,
  grain (frappe_doc_name)
);

-- JOINED layer for composition. See diagnostic_report_satusehat.sql for the
-- full pattern explanation. p.composition_title is the frappe-side raw
-- input; f.title is what actually round-tripped in the FHIR body -- both
-- kept so gold can compare, same rationale as report_display in
-- diagnostic_report.

WITH parent AS (
    SELECT *
    FROM silver.composition_satusehat_data_clean
    QUALIFY ROW_NUMBER() OVER (PARTITION BY name ORDER BY modified DESC) = 1
),
fhir AS (
    SELECT *
    FROM silver.composition_fhir_resource
    QUALIFY ROW_NUMBER() OVER (PARTITION BY frappe_doc_name ORDER BY modified DESC) = 1
),
audit AS (
    SELECT *
    FROM silver.composition_satusehat_data_submission_audit
    QUALIFY ROW_NUMBER() OVER (PARTITION BY name ORDER BY modified DESC) = 1
)
...
SELECT
    ...
    -- frappe-side raw input, kept for comparison against f.title
    p.composition_title,
