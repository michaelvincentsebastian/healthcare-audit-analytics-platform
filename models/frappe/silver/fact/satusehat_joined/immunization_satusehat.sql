MODEL (
  name silver.immunization_satusehat,
  kind FULL,
  grain (frappe_doc_name)
);

-- JOINED layer for immunization. See diagnostic_report_satusehat.sql for
-- the full pattern explanation. `dose_number` has no FHIR fragment
-- equivalent (not part of the flattened view), kept from parent as-is.

WITH parent AS (
    SELECT *
    FROM silver.immunization_satusehat_data_clean
    QUALIFY ROW_NUMBER() OVER (PARTITION BY name ORDER BY modified DESC) = 1
),
fhir AS (
    SELECT *
    FROM silver.immunization_fhir_resource
    QUALIFY ROW_NUMBER() OVER (PARTITION BY frappe_doc_name ORDER BY modified DESC) = 1
),
audit AS (
    SELECT *
    FROM silver.immunization_satusehat_data_submission_audit
    QUALIFY ROW_NUMBER() OVER (PARTITION BY name ORDER BY modified DESC) = 1
)
